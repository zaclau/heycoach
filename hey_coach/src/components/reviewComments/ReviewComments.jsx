import React from 'react';

function ReviewComments({
    coacheeProfileUrl = 'https://en.wikipedia.org/wiki/File:Northern_cardinal_female_in_CP_(02035).jpg',
    coacheeName = 'Anonymous',
    reviewDescription = 'No review provided.',
    reviewScore = 0
}) {
    // Inline styles for image sizing, shape, and margin
    const profileImageStyle = {
        width: '50px',        // Set image width
        height: '50px',       // Set image height
        borderRadius: '50%',  // Make it circular
        objectFit: 'cover',   // Ensure the image covers the area without distortion
        marginRight: '16px',  // Add right margin
    };

    return (
        <div className="container my-5">
            <div className="row justify-content-center">
                <div className="d-flex align-items-center">
                    <img
                        className="shadow"
                        src={coacheeProfileUrl}
                        alt={`${coacheeName}'s profile`}
                        style={profileImageStyle}  // Apply inline styles
                    />
                    <div>
                        <div className="fw-semibold">{coacheeName}</div>
                        <div className="fw-light">rating: {reviewScore} / 5</div>
                        <div>{reviewDescription}</div>
                    </div>
                </div>
            </div>
        </div>
    );
}

export default ReviewComments;


// function ReviewComments({
//     coacheeProfileUrl = 'https://en.wikipedia.org/wiki/File:Northern_cardinal_female_in_CP_(02035).jpg',
//     coacheeName = 'Anonymous',
//     reviewDescription = 'No review provided.',
//     reviewScore = 0
// }) {
//     // Inline styles for image sizing and shape
//     const profileImageStyle = {
//         width: '50px',        // Set image width
//         height: '50px',       // Set image height
//         borderRadius: '50%',   // Make it circular
//         objectFit: 'cover'     // Ensure the image covers the area without distortion
//     };
//
//     return (
//         <div className="container my-5">
//             <div className="row justify-content-center">
//                 <div className="col-1.5 d-flex align-items-center">
//                     <img
//                         className="shadow"
//                         src={coacheeProfileUrl}
//                         alt={`${coacheeName}'s profile`}
//                         style={profileImageStyle}  // Apply inline styles
//                     />
//                 </div>
//                 <div className="col-8">
//                     <div className="row justify-content-start">
//                         <div className="d-flex">
//                             <div className="me-2 fw-semibold">{coacheeName}</div>
//                             <div className="me-2 fw-light">rating: {reviewScore} / 5</div>
//                         </div>
//                     </div>
//                     <div className="row">
//                         <div className="col">
//                             {reviewDescription}
//                         </div>
//                     </div>
//                 </div>
//             </div>
//         </div>
//     );
// }
//
// export default ReviewComments;


// import React from 'react';
//
// function ReviewComments({
//     coacheeProfileUrl = 'https://en.wikipedia.org/wiki/File:Northern_cardinal_female_in_CP_(02035).jpg',
//     coacheeName = 'Anonymous', // Default name if not provided
//     reviewDescription = 'No review provided.', // Default text if the description is empty
//     reviewScore = 0
// }) {
//     return (
//         <div className="container my-5">
//             <div className="row justify-content-center">
//                 <div className="col-1 d-flex align-items-center">
//                     {/* Added alt attribute for accessibility */}
//                     <img className="rounded-circle shadow" src={coacheeProfileUrl} alt={`${coacheeName}'s profile`} />
//                 </div>
//                 <div className="col-10">
//                     <div className="row justify-content-start">
//                         <div className="d-flex">
//                             <div className="me-2 fw-semibold">{coacheeName}</div>
//                             {/* Now using the reviewDate prop */}
//                             <div className="me-2 fw-light">rating: {reviewScore} / 5</div>
//                         </div>
//                     </div>
//                     <div className="row">
//                         <div className="col">
//                             {reviewDescription}
//                         </div>
//                     </div>
//                 </div>
//             </div>
//         </div>
//     );
// }
//
// export default ReviewComments;

//############################################
// Version w props
//############################################

// import React from 'react';
// function ReviewComments({ coacheeProfileUrl, coacheeName, reviewDescription, }) {
//     return (
//         <div className="container my-5">
//             <div className="row justify-content-center">
//                 <div className="col-1 d-flex align-items-center">
//                     <img className="rounded-circle shadow ratio ratio-1x1" src={coacheeProfileUrl}></img>
//                 </div>
//                 <div className="col-10">
//                     <div className="row justify-content-start">
//                         <div className="d-flex">
//                             <div className="me-2 fw-semibold">{coacheeName}</div>
//                             <div className="me-2 fw-light">16 Sept 2023</div>
//                         </div>
//                     </div>
//                     <div className="row">
//                         <div className="col">
//                             {reviewDescription}
//                         </div>
//                     </div>
//                 </div>
//             </div>
//
//         </div>
//     )
//
// }
//
// export default ReviewComments;

// import React from 'react';
// function ReviewComments() {
//     return (
//         <div className="container my-5">
//             <div className="row justify-content-center">
//                 <div className="col-1 d-flex align-items-center">
//                     <img className="rounded-circle shadow ratio ratio-1x1" src="https://img.fruugo.com/product/5/43/182887435_max.jpg"></img>
//                 </div>
//                 <div className="col-10">
//                     <div className="row justify-content-start">
//                         <div className="d-flex">
//                             <div className="me-2 fw-semibold">John Cena</div>
//                             <div className="me-2 fw-light">16 Sept 2023</div>
//                         </div>
//                     </div>
//                     <div className="row">
//                         <div className="col">
//                             This is my review of our session. ore obscure Latin words, consectetur, from a Lorem Ipsum passage, and going through the cites of the word in classical literature, discovered the undoubtable source. Lorem Ipsum comes from sections 1.10.32 and 1.10.33 of "de Finibus Bonorum et Malorum" (The Extremes of Good and Evil) by Cicero, written in 45 BC. This book is a treatise on the theory of
//                         </div>
//                     </div>
//                 </div>
//             </div>
//
//         </div>
//     )
//
// }
//
// export default ReviewComments;